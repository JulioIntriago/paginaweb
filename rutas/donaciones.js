...
.get(function(request,response){
    ...
    let arreglo=[];
    basededatos.ref('donaciones').once('value').then(datos => {
...
.post( function(request,response,next){
    ...
    basededatos.ref("donaciones/"+ request.body.id).set(request.body).then(p=>{
...
basededatos.ref('donacionesrepuestos/'+request.params.idx).once('value').then(dato=>{
    response.send(dato.val());
  })
})
//método DELETE se debe enviar identificación
.delete(function(request,response,next){
    basededatos.ref('donacionesrepuestos/'+ request.params.idx ).remove().then(p=>{
